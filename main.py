...
def main(arqFile, arqTeste, arqSaida):
    automato = ler_automato(arqFile)
    if automato is None:
        return
    entradas = ler_entradas(arqTeste)
    if entradas is None:
        return
    resultados = []
    ...
    with open(arqSaida, 'w', newline='', encoding='utf-8') as file:
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("Uso: python automato.py <arqFile.aut> <arqTeste.in> <arqSaida.out>")
    else:
